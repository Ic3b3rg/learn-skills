---
name: zettelkasten
description: Produces atomic, linked markdown notes (Zettel) that capture one idea each, written in the user's own words, and cross-linked by topic. Each note carries a citation to the source artefact (code, docs, prior notes). Use when the user wants to turn a learning session into durable knowledge, when capturing insights from a /feynman or /elaboration session, when "I want notes I can come back to", or anytime they say "zettelkasten this", "save this as notes", "make me Zettel".
---
...
A generative skill that turns a session's insight into a small set of **atomic notes** under `learn/zettel/`, each capturing exactly one idea, written in the user's own words, with citations to the artefact and links to related notes. The notes outlast the session and compose into a growing knowledge base.

## Quick start

```
User: /zettelkasten "event sourcing"
...
After the session, files exist at `learn/zettel/<slug>.md`, each containing one note with frontmatter, body, citations, and links to other notes.
...
5. **Wording check.** Compare the body to known docs/code. If it is a near-quote, ask the user to restate. Real Zettel are paraphrases — paraphrasing is itself an act of learning.
6. **Link suggestion (gentle).** After writing the note, check `learn/zettel/` for existing notes with overlapping tags. Suggest 1–3 candidate links — the user accepts or rejects each. Never auto-link.
...
- All notes live under `learn/zettel/`. No deeper nesting.
...
- **Don't author body prose for the user** — the agent prompts, verifies, formats; never writes the user's idea for them, because Zettel that aren't in the user's own words don't trigger the paraphrase-as-learning act that justifies the format.
...
- **Don't mix Zettel with raw notes or sketches** — atomic-only is what makes the collection composable; other captures (drafts, outlines, scratch) go elsewhere.
