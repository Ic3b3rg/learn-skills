...
Operational policy for how skills in this repo verify claims made by the user (and by themselves) against external knowledge. Companion to **Principle 4: Source fidelity** in [CONTEXT.md](https://github.com/Ic3b3rg/learn-skills/blob/master/CONTEXT.md). Linked from every skill that performs verification.
...
1. **Read local files** using the available file-reading tools.
2. **Retrieve indexed library documentation at the pinned version**, when available (for example, Context7).
3. **Fetch a supplied URL** with the available web retrieval tool, or retrieve canonical documentation directly when no suitable documentation index is available.
4. **Search the web** only when the canonical URL is unknown and must be discovered.
